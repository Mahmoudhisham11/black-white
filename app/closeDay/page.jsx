...
// المستخدم الذي يُعتبر Admin (لمشاهدة أرباح التقفيل فقط)
const ADMIN_USER = "mostafabeso10@gmail.com";

const getSaleProfit = (sale) => {
  const directProfit = Number(sale?.profit);
  if (!isNaN(directProfit)) return directProfit;

  if (!Array.isArray(sale?.cart)) return 0;

  return sale.cart.reduce((sum, item) => {
    const qty = Number(item?.quantity ?? 0);
    const buyPrice = Number(item?.buyPrice ?? 0);
    const sellPrice = Number(item?.sellPrice ?? 0);

    if ([qty, buyPrice, sellPrice].some((v) => isNaN(v))) return sum;
...
function CloseDayContent() {
  ...
  const totals = useMemo(() => {
    ...
    const profit = salesArr.reduce((sum, s) => sum + getSaleProfit(s), 0);
