...
export default function Profit() {
  ...
  const fetchData = async () => {
    ...
    const deletedSnap = await getDocs(
      query(collection(db, "deletedProducts"), where("shop", "==", shop))
    );
    const deletedArr = deletedSnap.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
    ...
    const resetLogsSnap = await getDocs(
      query(collection(db, "deletedResetLogs"), where("shop", "==", shop))
    );
    const resetLogs = resetLogsSnap.docs.map((doc) => doc.data());
    const lastReset =
      resetLogs.length > 0
        ? resetLogs.reduce((prev, curr) =>
            prev.resetAt.seconds > curr.resetAt.seconds ? prev : curr
          )
        : null;
    const lastResetDate = lastReset?.resetAt?.toDate
      ? lastReset.resetAt.toDate()
      : null;

    // حساب إجمالي المرتجعات بعد آخر تصفير
    const totalDeleted = deletedArr.reduce((sum, p) => {
      const deletedDate = p.createdAt?.toDate
        ? p.createdAt.toDate()
        : new Date();
      if (!lastResetDate || deletedDate > lastResetDate) {
        return (
          sum + (Number(p.buyPrice) || 0) * (Number(p.deletedTotalQty) || 0)
        );
      }
      return sum;
    }, 0);
  ...
  const handleWithdraw = async () => {
    if (!withdrawPerson || !withdrawAmount)
      return alert("اختر الشخص واكتب المبلغ");
  ...
  const handlePay = async () => {
    ...
    if (amount > remainingDebt)
      return alert(`المبلغ أكبر من المبلغ المستحق: ${remainingDebt}`);
            ...
            <tbody>
              {withdraws.map((w) => (
                <tr key={w.id}>
                  ...
                  <td>
                    {formatDate(parseDate(w.date) || parseDate(w.createdAt))}
                  </td>
                  <td>{w.notes || ""}</td>
